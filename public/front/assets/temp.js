...
; /* Start:"a:4:{s:4:"full";s:65:"/local/templates/texx/vendor/jquery/jquery.min.js?161591625497162";s:6:"source";s:49:"/local/templates/texx/vendor/jquery/jquery.min.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
...
ï»¿// Ion.RangeSlider | version 2.2.0 | https://github.com/IonDen/ion.rangeSlider
...
/*				if (e.item.index == e.item.count-1){
	...
	//Ð»Ð¸ÑÑ‚Ð°Ð½Ð¸Ðµ Ð½Ð° ÐºÐ»Ð°Ð²Ð¸ÑˆÐ¸ ÐºÐ»Ð°Ð²Ð¸Ð°Ñ‚ÑƒÑ€Ñ‹
	...
	if ($('.scrolled-content').length){
		...
		$('.anchors a').click(function(e){
			...
			if (!anchor_animate){
	        	...
	        	// Ð¿Ñ€Ð¾Ð¼Ð¾Ñ‚ÐºÐ° Ð¼ÐµÐ½ÑŽÑˆÐºÐ¸ Ð´Ð¾ Ð°ÐºÑ‚Ð¸Ð²Ð½Ð¾Ð¹ ÑÑÑ‹Ð»ÐºÐ¸
  ...
  //                       		$('a[href$='+id+']').parent().addClass('active');
	...
	// main Ð»Ð¾Ð³Ð¸ÐºÐ° Ð»Ð¸ÑÑ‚Ð°Ð½Ð¸Ñ Ð³Ð»Ð°Ð²Ð½Ð¾Ð¹ ÑÑ‚Ñ€Ð°Ð½Ð¸Ñ†Ñ‹ Ð½Ð°Ð¶Ð°Ñ‚Ð¸Ðº Ð½Ð° ÐºÐ½Ð¾Ð¿ÐºÑƒ.

	doc.on('click', '.next-main-link', function(e){
		e.preventDefault();

		var main = $('.main-content'); // ÑÐ¾Ñ…Ñ€Ð°Ð½ÑÐµÐ¼ ÐºÐ¾Ð½Ñ‚ÐµÐ¹Ð½ÐµÑ€ Ð² Ð¿ÐµÑ€ÐµÐ¼ÐµÐ½Ð½ÑƒÑŽ
		var translate = $('.main-container').css('transform').split(/[()]/)[1]; // Ñ‡Ð¸Ñ‚Ð°ÐµÐ¼ ÑÐ¼ÐµÑ‰ÐµÐ½Ð¸Ðµ
		var posy = translate.split(',')[5]; // ÑÐ¼ÐµÑ‰ÐµÐ½Ð¸Ðµ Ð½Ð°Ð²ÐµÑ€Ñ…
		var _this = $(this); // ÑÐ¾Ñ…Ñ€Ð¾Ð½ÑÐµÐ¼ ÑÑÑ‹Ð»ÐºÑƒ
		...
		if (_this.index($('.next-main-link')) == 0){
			...
			// Ð´Ð°Ð»ÐµÐµ Ð¿Ñ€Ð¾Ð²ÐµÑ€ÑÐµÐ¼ Ð½Ðµ Ð¿Ð¾ÑÐ»ÐµÐ´Ð½Ð¸Ð¹ Ð»Ð¸ ÑÑ‚Ð¾ ÑÐ»ÐµÐ¼ÐµÐ½Ñ‚, Ñ‚Ð°Ðº ÐºÐ°Ðº Ð¿Ð¾ÑÐ»ÐµÐ´Ð½Ð¸Ð¹ Ð´Ð¾Ð»Ð¶ÐµÐ½ Ð¿Ð¾Ð·Ð²Ñ€Ð°Ñ‰Ð°Ñ‚ÑŒ Ð½Ð°Ñ Ð² Ð½Ð°Ñ‡Ð°Ð»Ð¾
	...
	if ($('.calc-sliders').length){
		...
		options = {
			...
			yAxis: {
				title: {
					text: 'Ð¡Ñ€ÐµÐ´ÑÑ‚Ð²Ð° (Ñ€ÑƒÐ±.)'
				}
			},
			xAxis: {
				title:{
					text: 'Ð’Ñ€ÐµÐ¼Ñ (Ð¼ÐµÑ.)'
				},
				tickInterval: 1
			},
			plotOptions: {
		...
		$('#amount').ionRangeSlider({
			...
			onStart: function(data) {
				$('#amount').parents('.calc-row').find('.value').text(data.from_pretty + ' ÐµÐ´.');
				amount = data.from;
		...
		$('#run').ionRangeSlider({
			...
			onStart: function(data) {
				...
				$('#run').parents('.calc-row').find('.value').text(data.from_pretty + ' ÐºÐ¼.');
				// $('#run').parents('.calc-row').find('.value').text(value + ' ÐºÐ¼.');
				run = data.from/30;
				fuelRate = calcFuilRate(run, cost, drain);
				allFuel = calcAllFuel(fuelRate, amount);
				calceconomy();
		...
		$('#cost').ionRangeSlider({
		    ...
		    grid: false,
			onStart: function(data) {
				$('#cost').parents('.calc-row').find('.value').text(data.from_pretty + ' Ñ€ÑƒÐ±/Ð».');
		...
		$('#drain').ionRangeSlider({
		    ...
		    grid: false,
			onStart: function(data) {
				$('#drain').parents('.calc-row').find('.value').text(data.from_pretty + ' Ð».');
				drain = data.from;
				fuelRate = calcFuilRate(run, cost, drain);
				allFuel = calcAllFuel(fuelRate, amount);
				calceconomy();
		...
		function calceconomy(){
			...
			$('#run_economy').text(addCommas(parseInt(run))+" Ñ€ÑƒÐ±./Ð¼ÐµÑ.");
			$('#sell_economy').text(addCommas(parseInt(sell))+" Ñ€ÑƒÐ±./Ð¼ÐµÑ.");
			$('#opti_economy').text(addCommas(parseInt(perOne))+" Ñ€ÑƒÐ±./Ð¼ÐµÑ.");
			$('#all_economy').text(addCommas(parseInt(all))+" Ñ€ÑƒÐ±./Ð¼ÐµÑ.");
			$('#once_budget').text(addCommas(parseInt(once_budget)) + " Ñ€ÑƒÐ±.");
			$('#every_month_budget').text(addCommas(parseInt(month)) + " Ñ€ÑƒÐ±./Ð¼ÐµÑ.");
			$('#payback').text(addCommas(parseInt(payback)) + " Ð¼ÐµÑ.");
	...
	if ($('.partner-map').length){
		...
		var areas = [
			{countryId: "RU-AD", text: "ÐÐ´Ñ‹Ð³ÐµÐ¹ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-ALT", text: "ÐÐ»Ñ‚Ð°Ð¹ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-AMU", text: "ÐÐ¼ÑƒÑ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-BRY", text: "Ð‘Ñ€ÑÐ½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-CHE", text: "Ð§ÐµÐ»ÑÐ±Ð¸Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-CHU", text: "Ð§ÑƒÐºÐ¾Ñ‚ÐºÐ°"},
			{countryId: "RU-KC", text: "ÐšÐ°Ñ€Ð°Ñ‡Ð°ÐµÐ²Ð¾-Ð§ÐµÑ€ÐºÐµÑÐ¸Ñ"},
			{countryId: "RU-KDA", text: "ÐšÑ€Ð°ÑÐ½Ð¾Ð´Ð°Ñ€ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-KEM", text: "ÐšÐµÐ¼ÐµÑ€Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KLU", text: "ÐšÐ°Ð»ÑƒÐ¶ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KHA", text: "Ð¥Ð°Ð±Ð°Ñ€Ð¾Ð²ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-KL", text: "ÐšÐ°Ð»Ð¼Ñ‹ÐºÐ¸Ñ"},
			{countryId: "RU-KRS", text: "ÐšÑƒÑ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-MOW", text: "ÐœÐ¾ÑÐºÐ²Ð°"},
			{countryId: "RU-MOS", text: "ÐœÐ¾ÑÐºÐ²Ð°"},
			{countryId: "RU-NGR", text: "Ð‘Ñ€ÑÐ½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-PNZ", text: "ÐŸÐµÐ½Ð·ÐµÐ½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-ROS", text: "Ð Ð¾ÑÑ‚Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-RYA", text: "Ð ÑÐ·Ð°Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-SAM", text: "Ð¡Ð°Ð¼Ð°Ñ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-SMO", text: "Ð¡Ð¼Ð¾Ð»ÐµÐ½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-SPE", text: "Ð¡Ð°Ð½ÐºÑ‚-ÐŸÐµÑ‚ÐµÑ€Ð±ÑƒÑ€Ð³"},
			{countryId: "RU-SAR", text: "Ð¡Ð°Ñ€Ð°Ñ‚Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-STA", text: "Ð¡Ñ‚Ð°Ð²Ñ€Ð¾Ð¿Ð¾Ð»ÑŒÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-TVE", text: "Ð¢Ð²ÐµÑ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TYU", text: "Ð¢ÑŽÐ¼ÐµÐ½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-UD", text: "Ð£Ð´Ð¼ÑƒÑ€Ñ‚Ð¸Ñ"},
			{countryId: "RU-ULY", text: "Ð£Ð»ÑŒÑÐ½Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-VGG", text: "Ð’Ð¾Ð»Ð³Ð¾Ð³Ñ€Ð°Ð´ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-VLA", text: "Ð’Ð»Ð°Ð´Ð¸Ð¼Ð¸Ñ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-VOR", text: "Ð’Ð¾Ñ€Ð¾Ð½ÐµÐ¶ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-YAR", text: "Ð¯Ñ€Ð¾ÑÐ»Ð°Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-ZAB", text: "Ð—Ð°Ð±Ð°Ð¹ÐºÐ°Ð»ÑŒÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-SVE", text: "Ð¡Ð²ÐµÑ€Ð´Ð»Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TAM", text: "Ð¢Ð°Ð¼Ð±Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-PER", text: "ÐŸÐµÑ€Ð¼ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-PRI", text: "ÐŸÑ€Ð¸Ð¼Ð¾Ñ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-BA", text: "Ð‘Ð°ÑˆÐºÐ¾Ñ€Ñ‚Ð¾ÑÑ‚Ð°Ð½"},
			{countryId: "RU-BEL", text: "Ð‘ÐµÐ»Ð³Ð¾Ñ€Ð¾Ð´ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TA", text: "Ð¢Ð°Ñ‚Ð°Ñ€ÑÑ‚Ð°Ð½"},
			{countryId: "RU-DA", text: "Ð”Ð°Ð³ÐµÑÑ‚Ð°Ð½"},
			{countryId: "RU-MAG", text: "ÐœÐ°Ð³Ð°Ð´Ð°Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-NIZ", text: "ÐÐ¸Ð¶ÐµÐ³Ð¾Ñ€Ð¾Ð´ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-ORE", text: "ÐžÑ€ÐµÐ½Ð±ÑƒÑ€Ð³ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KOS", text: "ÐšÐ¾ÑÑ‚Ñ€Ð¾Ð¼ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KU", text: "ÐšÑƒÑ€Ð³Ð°Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KIR", text: "ÐšÐ¸Ñ€Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-LIP", text: "Ð›Ð¸Ð¿ÐµÑ‚ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TY", text: "Ð¢Ñ‹Ð²Ð°"},
			{countryId: "RU-CU", text: "Ð§ÑƒÐ²Ð°ÑˆÐ¸Ñ"},
			{countryId: "RU-AL", text: "Ð“Ð¾Ñ€Ð½Ñ‹Ð¹ Ð°Ð»Ñ‚Ð°Ð¹"},
			{countryId: "RU-IN", text: "Ð˜Ð½Ð³ÑƒÑˆÐµÑ‚Ð¸Ñ"},
			{countryId: "RU-OMS", text: "ÐžÐ¼ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-PSK", text: "ÐŸÑÐºÐ¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-NVS", text: "ÐÐ¾Ð²Ð¾ÑÐ¸Ð±Ð¸Ñ€ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-ORL", text: "ÐžÑ€Ð»Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TOM", text: "Ð¢Ð¾Ð¼ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-TUL", text: "Ð¢ÑƒÐ»ÑŒÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-KHM", text: "Ð¥Ð°Ð½Ñ‚Ñ‹-Ð¼Ð°Ð½ÑÐ¸Ð¹ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-KO", text: "Ð ÐµÑÐ¿ÑƒÐ±Ð»Ð¸ÐºÐ° ÐšÐ¾Ð¼Ð¸"},
			{countryId: "RU-KAM", text: "ÐšÐ°Ð¼Ñ‡Ð°Ñ‚ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-KR", text: "Ð ÐµÑÐ¿ÑƒÐ±Ð»Ð¸ÐºÐ° ÐšÐ°Ñ€ÐµÐ»Ð¸Ñ"},
			{countryId: "RU-KK", text: "Ð¥Ð°ÐºÐ°ÑÐ¸Ñ"},
			{countryId: "RU-MUR", text: "ÐœÑƒÑ€Ð¼Ð°Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-MO", text: "ÐœÐ¾Ñ€Ð´Ð¾Ð²Ð¸Ñ"},
			{countryId: "RU-ARK", text: "ÐÑ€Ñ…Ð°Ð½Ð³ÐµÐ»ÑŒÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-AST", text: "ÐÑÑ‚Ñ€Ð°Ñ…Ð°Ð½ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-NEN", text: "ÐÐµÐ½ÐµÑ‚ÑÐºÐ¸Ð¹ Ð°Ð²Ñ‚Ð¾Ð½Ð¾Ð¼Ð½Ñ‹Ð¹ Ð¾ÐºÑ€ÑƒÐ³"},
			{countryId: "RU-SE", text: "Ð¡ÐµÐ²ÐµÑ€Ð½Ð°Ñ Ð¾ÑÐµÑ‚Ð¸Ñ"},
			{countryId: "RU-SA", text: "Ð¯ÐºÑƒÑ‚Ð¸Ñ"},
			{countryId: "RU-SAK", text: "Ð¡Ð°Ñ…Ð°Ð»Ð¸Ð½"},
			{countryId: "RU-KYA", text: "ÐšÑ€Ð°ÑÐ½Ð¾ÑÑ€ÑÐºÐ¸Ð¹ ÐºÑ€Ð°Ð¹"},
			{countryId: "RU-YAN", text: "Ð¯Ð¼Ð°Ð»Ð¾-ÐÐµÐ½ÐµÑ†ÐºÐ¸Ð¹ Ð°Ð²Ñ‚Ð¾Ð½Ð¾Ð¼Ð½Ñ‹Ð¹ Ð¾ÐºÑ€ÑƒÐ³"},
			{countryId: "RU-VLG", text: "Ð’Ð¾Ð»Ð¾Ð³Ð¾Ð´ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-LEN", text: "Ð›ÐµÐ½Ð¸Ð½Ð³Ñ€Ð°Ð´ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-IRK", text: "Ð˜Ñ€ÐºÑƒÑ‚ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-IVA", text: "Ð˜Ð²Ð°Ð½Ð¾Ð²ÑÐºÐ°Ñ Ð¾Ð±Ð»Ð°ÑÑ‚ÑŒ"},
			{countryId: "RU-BU", text: "Ð‘ÑƒÑ€ÑÑ‚Ð¸Ñ"},
			{countryId: "RU-CE", text: "Ð§ÐµÑ‡Ð½Ñ"}
		]
...
/*
    ...
    var light_costs = [];
	var light_cost = new LightCost("Ð›ÐµÐ³ÐºÐ¾Ð²Ð¾Ð¹");
	...
	var heavy_cost = new LightCost("Ð“Ñ€ÑƒÐ·Ð¾Ð²Ð¾Ð¹");
	// function HeavyCost() {
	// 	selfcost: 0,
	// 	mountcost: 0,
	// 	allcost: 0,
	...
	doc.on('click', '.addCalculate', function (e) {
        ...
        light_cost = new LightCost("Ð›ÐµÐ³ÐºÐ¾Ð²Ð¾Ð¹");
        heavy_cost = new LightCost("Ð“Ñ€ÑƒÐ·Ð¾Ð²Ð¾Ð¹");
    ...
    // ÐºÐ¾ÑÑ‚Ñ‹Ð»ÑŒ Ð´Ð»Ñ Ð°Ð½Ð´Ñ€Ð¾Ð¸Ð´ÑÐºÐ¾Ð¹ ÐºÐ»Ð°Ð²Ñ‹
...
// Ñ‚Ð°Ñ‡ Ð»Ð¸ÑÑ‚Ð°Ð½Ð¸Ðµ Ð½Ð° Ð³Ð»Ð°Ð²Ð½Ð¾Ð¹
...
// Ð›Ð¾Ð³Ð¸ÐºÐ° Ð»Ð¸ÑÑ‚Ð°Ð½Ð¸Ñ Ð½Ð° ÐºÐ¾Ð»ÐµÑÐ¸ÐºÐ¾ Ð¼Ñ‹ÑˆÐºÐ¸
...
//Ð»Ð¸ÑÑ‚Ð°Ð½Ð¸Ðµ Ð³Ð»Ð°Ð²Ð½Ð¾Ð¹ Ð½Ð° ÐºÐ»Ð°Ð²Ð¸ÑˆÐ¸ ÐºÐ»Ð°Ð²Ð¸Ð°Ñ‚ÑƒÑ€Ñ‹
$(document).on('keydown', function(e){
		if (e.keyCode == 38){
			if ($('#mainpage').length){
				init_scroll(event, 1);
...
function init_scroll(event, delta) {
   ...
   //	setTimeout(function(){
		if (deltaOfInterest < 0) {
			// ÐµÑÐ»Ð¸ Ð²Ð½Ð¸Ð· Ð¿Ñ€Ð¾ÑÑ‚Ð¾ Ð½Ð°Ð¶Ð¸Ð¼Ð°ÐµÐ¼ Ð½Ð° ÐºÐ½Ð¾Ð¿Ð¾Ñ‡ÐºÑƒ
			if (!$('.next-main-link.active').hasClass('last')){
				$('.next-main-link.active').click();
			}
	   	} else {
	   		// Ð° Ð²Ð¾Ñ‚ Ð² Ð²ÐµÑ€Ñ… Ð¿Ñ€Ð¸Ð´ÐµÑ‚ÑÑ Ð½ÐµÐ¼Ð½Ð¾Ð³Ð¾ Ð¸Ð·ÑŠÐµÐ±Ð½ÑƒÑ‚ÑŒÑÑ.
	   		mainScrollUp();
	   	}
   	//}, 0)
...
/* End */
;; /* /local/templates/texx/vendor/jquery/jquery.min.js?161591625497162*/
; /* /local/templates/texx/vendor/jquery.touchSwipe.min.js?161591625420430*/
; /* /local/templates/texx/vendor/fontawesome.js?16159162549500*/
; /* /local/templates/texx/vendor/select2/select2.min.js?161591625466664*/
; /* /local/templates/texx/vendor/bootstrap/bootstrap.min.js?161591625437331*/
; /* /local/templates/texx/vendor/owlCarousel/owl.carousel.min.js?161591625442766*/
; /* /local/templates/texx/vendor/jqueryHammer/hammer.js?161591625420726*/
; /* /local/templates/texx/vendor/iscroll/iscroll.js?162825997055104*/
; /* /local/templates/texx/vendor/iscroll/iscroll-probe.js?162826207555782*/
; /* /local/templates/texx/vendor/jqueryHammer/jquery.hammer.js?1615916254988*/
; /* /local/templates/texx/vendor/jqueryMousewheel/jquery.mCustomScrollbar.concat.min.js?161591625445479*/
; /* /local/templates/texx/vendor/ionRangeSlider/ion.rangeSlider.min.js?161591625440897*/
; /* /local/templates/texx/vendor/highcharts/highcharts.js?1615916254205069*/
; /* /local/templates/texx/js/jquery.cookie.js?16159162543121*/
; /* /local/templates/texx/app.js?162826306153333*/
}else{
 console.log("The mobile");
}